...
function LoadPlayer(identity) {
    ...
    busyIndicator.show();
    $.get("/Activity/GetClubCustomPlayerUser/?idNum=" + identity +
        ...
        function (data) {
            ...
            if (data) {
                ...
                if (data.MedicalCert) {
                    $("#buildForm-body-medicalCert").after($("<a>", { href: data.MedicalCert, "class": "glyphicon glyphicon-eye-open glyph-btn", target: "_blank" }));
                }
                if (data.InsuranceCert) {
                    $("#buildForm-body-insuranceCert").after($("<a>", { href: data.InsuranceCert, "class": "glyphicon glyphicon-eye-open glyph-btn", target: "_blank" }));
                }
            ...
            if (existingItem.length <= 0) {
                teamsDropdown.append(
                    $("<option>",
                        {
                            value: item.TeamId,
                            text: $("<div/>").html(item.TeamName).text(),
